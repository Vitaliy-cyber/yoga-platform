...
test.describe("Atomic refresh CSRF with invalid bearer (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("invalid bearer must not bypass CSRF requirement", async ({ request }) => {
    const token = makeIsolatedToken("refresh-csrf-invalid-bearer");
    ...
    assertNo5xx(refreshRes.status(), "refresh invalid bearer");
