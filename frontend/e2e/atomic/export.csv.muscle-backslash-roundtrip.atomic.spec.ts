...
test.describe("Atomic export CSV muscle backslash roundtrip (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("CSV export+import preserves muscle names with backslashes/commas/colons", async () => {
    const accessToken = (await loginWithToken(makeIsolatedToken("export-csv-muscle-bslash"))).accessToken;
    ...
    const muscleName = `back\\slash,colon:comma`;
    ...
    const code = `CSVMB_${Date.now().toString(36).slice(-8)}`.slice(0, 20);
    const createPoseRes = await authedFetch(accessToken, "/api/v1/poses", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        code,
        name: "CSV muscle backslash pose",
    ...
    try {
      ...
      // muscles column should include escaped sequences
      expect(cols[7]).toContain("\\\\");
      expect(cols[7]).toContain("\\,");
      expect(cols[7]).toContain("\\:");
      ...
      const fetched = (await fetchPoseRes.json()) as { muscles?: Array<{ muscle_name?: string }>; id?: number };
