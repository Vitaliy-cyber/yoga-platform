...
test.describe("Atomic logout invalid CSRF cross-user fallback (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("invalid CSRF with cookie A + body B revokes only B", async ({ request }) => {
    const tokenA = makeIsolatedToken("logout-invalid-csrf-a");
    const tokenB = makeIsolatedToken("logout-invalid-csrf-b");
