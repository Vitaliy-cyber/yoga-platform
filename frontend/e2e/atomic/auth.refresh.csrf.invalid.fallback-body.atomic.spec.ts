...
test.describe("Atomic refresh invalid CSRF uses body token (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("invalid CSRF should not block body refresh token", async ({ request }) => {
    const token = makeIsolatedToken("refresh-invalid-csrf-body");
    ...
    assertNo5xx(refreshRes.status(), "refresh invalid csrf fallback");
