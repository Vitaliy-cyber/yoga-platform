...
test.describe("Atomic logout CSRF requirement (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("logout with refresh cookie requires X-CSRF-Token", async ({ request }) => {
    const token = makeIsolatedToken("auth-logout-csrf");
