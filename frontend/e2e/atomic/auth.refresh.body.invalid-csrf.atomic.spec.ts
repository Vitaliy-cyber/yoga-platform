...
test.describe("Atomic refresh body token with invalid CSRF (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("body refresh ignores invalid CSRF header when no cookie", async ({ request }) => {
    const token = makeIsolatedToken("refresh-body-invalid-csrf");
    ...
    assertNo5xx(refreshRes.status(), "refresh body invalid csrf");
