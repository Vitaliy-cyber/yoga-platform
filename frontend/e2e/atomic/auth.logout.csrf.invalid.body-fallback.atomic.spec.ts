...
test.describe("Atomic logout invalid CSRF body fallback (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("invalid CSRF should not block body refresh token logout", async ({ request }) => {
    const token = makeIsolatedToken("logout-invalid-csrf-body");
    ...
    assertNo5xx(logoutRes.status(), "logout invalid csrf body fallback");
    expect(logoutRes.status()).toBe(200);

    const refreshB = await bare.post(`${API_BASE_URL}/api/v1/auth/refresh`, {
      data: { refresh_token: loginBJson.refresh_token },
    });
    assertNo5xx(refreshB.status(), "refresh after logout (body token)");
    expect(refreshB.status()).toBe(401);

    const refreshA = await bare.post(`${API_BASE_URL}/api/v1/auth/refresh`, {
      data: { refresh_token: loginAJson.refresh_token },
    });
    assertNo5xx(refreshA.status(), "refresh cookie token still valid");
