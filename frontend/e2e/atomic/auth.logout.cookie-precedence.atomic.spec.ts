...
test.describe("Atomic logout cookie precedence (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("logout uses cookie when body token differs", async () => {
    ...
    const tokenA = makeIsolatedToken("logout-cookie-a");
    const tokenB = makeIsolatedToken("logout-cookie-b");

    const loginA = await loginWithContext(ctxA, tokenA);
    const loginB = await loginWithContext(ctxB, tokenB);

    const logoutRes = await ctxA.post(`${API_BASE_URL}/api/v1/auth/logout`, {
      data: { refresh_token: loginB.refresh_token },
      headers: { "X-CSRF-Token": loginA.csrf_token },
    });
    assertNo5xx(logoutRes.status(), "logout cookie precedence");
    ...
    assertNo5xx(refreshA.status(), "refresh after logout (cookie session)");
    expect(refreshA.status()).toBe(401);

    const refreshB = await bare.post(`${API_BASE_URL}/api/v1/auth/refresh`, {
      data: { refresh_token: loginB.refresh_token },
    });
    assertNo5xx(refreshB.status(), "refresh after logout (body session)");
