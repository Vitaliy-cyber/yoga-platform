...
test.describe("Atomic regenerate upload fallback: /generate retries transient 429", () => {
  test.describe.configure({ mode: "serial" });

  test("regenerate completes even if the first /generate request is rate-limited (429)", async ({ browser }) => {
    ...
    const token = makeIsolatedToken(`regen-generate-429-${Date.now()}`);
    const { accessToken } = await loginWithToken(token);
    expect(accessToken).toBeTruthy();
    await uiLoginWithToken(page, token);

    const code = `RG429_${Date.now().toString(36).slice(-10)}`.slice(0, 20);
    const createRes = await authedFetch(accessToken, "/api/v1/poses", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code, name: `Atomic Regen /generate 429 ${code}` }),
    });
    assertNo5xx(createRes.status, "create pose (regen /generate 429 suite)");
    ...
    // Ensure upload fallback is used (no schema_path).
    // Also make all /storage image fetches deterministic for both <img> and fetch().
    ...
    await page.route(`**/api/v1/poses/${poseId as number}`, async (route) => {
      ...
      assertNo5xx(real.status, "get pose (regen /generate 429 suite)");
    ...
    try {
      ...
      await page.getByTestId("pose-regenerate-feedback").fill("atomic: retry 429 on /generate");
      await page.getByTestId("pose-regenerate-start").click();

      // Should automatically retry without a second click.
      await expect.poll(() => generateCalls, { timeout: 25_000 }).toBe(2);
