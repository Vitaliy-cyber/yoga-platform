...
test.describe("Atomic regenerate: apply-generation retries transient 409", () => {
  test.describe.configure({ mode: "serial" });

  test("retries apply-generation on 409 and closes modal after success", async ({ page }) => {
    const poseId = getCorePoseIdA();
    test.skip(!poseId, "Core seed pose not available");

    const taskId = `atomic_apply_retry_${Date.now()}`;
    ...
    // Mark task as completed so the modal attempts apply-generation.
      ...
      async (route) => {
        ...
        if (applyCalls <= 2) {
          await route.fulfill({
            status: 409,
            headers: { "content-type": "application/json" },
            body: JSON.stringify({ detail: "Atomic: transient conflict" }),
          });
          return;
        }
        await route.fulfill({
    ...
    await page.getByTestId("pose-regenerate-feedback").fill("atomic: apply retries 409");
    await page.getByTestId("pose-regenerate-start").click();

    // Ensure we retried and eventually succeeded.
    await expect.poll(() => applyCalls, { timeout: 20_000 }).toBeGreaterThanOrEqual(3);

    // Modal should auto-close after successful apply.
