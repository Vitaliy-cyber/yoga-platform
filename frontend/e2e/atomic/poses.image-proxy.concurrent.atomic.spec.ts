...
async function uploadSchema(accessToken: string, poseId: number, i: number): Promise<number> {
  ...
  form.append("file", new Blob([bytes], { type: "image/png" }), `atomic-img-${i}.png`);
...
test.describe("Atomic pose image proxy concurrency (break-it; never 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  const iterations = getEnvInt("ATOMIC_IMAGE_PROXY_RACE_ITER", 240);
  const concurrency = Math.min(getEnvInt("ATOMIC_IMAGE_PROXY_RACE_CONCURRENCY", 16), 24);
  ...
  test.beforeAll(async () => {
    ...
    const code = `AIP_${suffix}`.slice(0, 20);
    const created = await authedFetch(accessToken, "/api/v1/poses", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code, name: "Atomic Image Proxy Race", description: "atomic" }),
    });
    assertNo5xx(created.status, "create pose (image proxy race)");
  ...
  test("GET proxy image under concurrent uploads never 5xx", async () => {
    if (!poseId) throw new Error("poseId not set");

    const tasks = Array.from({ length: iterations }, (_, i) => async () => {
      // Keep constant write pressure to provoke SQLite 'database is locked' in auth/pose reads.
      if (i % 5 === 0) {
      ...
      const res = await fetch(`${API_BASE_URL}/api/v1/poses/${poseId as number}/image/schema?i=${i}`, {
        method: "GET",
        headers: { Authorization: `Bearer ${accessToken}`, Accept: "image/*" },
      });
      assertNo5xx(res.status, `proxy image #${i}`);
