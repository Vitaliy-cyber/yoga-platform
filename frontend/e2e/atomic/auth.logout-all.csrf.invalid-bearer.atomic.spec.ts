...
test.describe("Atomic logout-all CSRF with invalid bearer (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("invalid bearer must not bypass CSRF requirement", async ({ request }) => {
    const token = makeIsolatedToken("logoutall-csrf-invalid-bearer");
    ...
    assertNo5xx(logoutAllRes.status(), "logout-all invalid bearer");
    expect(logoutAllRes.status()).toBe(403);
  });
});
