...
test.describe("Atomic logout-all revoked cookie fallback (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("revoked refresh cookie should not block body-based logout-all", async ({ request }) => {
    const token = makeIsolatedToken("logoutall-revoked-cookie-body");
    ...
    const logoutAllRes = await bare.post(`${API_BASE_URL}/api/v1/auth/logout-all`, {
      ...
      headers: {
        Cookie: `refresh_token=${first.refresh_token}`,
      },
    });
    assertNo5xx(logoutAllRes.status(), "logout-all fallback to body");
    expect(logoutAllRes.status()).toBe(200);

    const refreshSecond = await bare.post(`${API_BASE_URL}/api/v1/auth/refresh`, {
      data: { refresh_token: second.refresh_token },
    });
    assertNo5xx(refreshSecond.status(), "refresh after logout-all");
