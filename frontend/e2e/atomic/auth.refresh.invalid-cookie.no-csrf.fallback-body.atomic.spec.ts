...
test.describe("Atomic refresh invalid cookie + no CSRF (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("invalid refresh cookie must not block body refresh without CSRF", async ({ request }) => {
    const token = makeIsolatedToken("refresh-invalid-cookie-no-csrf");
    ...
    assertNo5xx(refreshRes.status(), "refresh invalid cookie no csrf");
