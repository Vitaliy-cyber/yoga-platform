...
test.describe("Atomic generate: apply-generation retries transient 503", () => {
  test.describe.configure({ mode: "serial" });

  test("retries apply-generation on 503 and closes modal after success", async ({ browser }) => {
    test.setTimeout(150_000);

    const token = makeIsolatedToken(`gen-apply-503-${Date.now()}`);
    ...
    assertNo5xx(createRes.status, "create pose (generate apply 503 suite)");
    ...
    assertNo5xx(meRes.status, "auth/me (generate apply 503)");
    ...
    const taskId = `atomic_generate_apply_503_${Date.now()}`;
    ...
    try {
      ...
      await page.getByTestId("pose-generate-notes").fill("atomic: apply retries 503");
