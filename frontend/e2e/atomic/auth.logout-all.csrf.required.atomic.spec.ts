...
test.describe("Atomic logout-all CSRF requirement (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("logout-all with refresh cookie requires X-CSRF-Token", async ({ request }) => {
    const token = makeIsolatedToken("auth-logout-all-csrf");
    ...
    assertNo5xx(ok.status(), "logout-all with csrf");
