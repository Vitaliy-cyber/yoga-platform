...
test.describe("Atomic logout-all missing CSRF does not revoke (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("logout-all missing CSRF leaves refresh token valid", async ({ request }) => {
    const token = makeIsolatedToken("logoutall-csrf-missing-no-revoke");
    ...
    const missingCsrf = await bare.post(`${API_BASE_URL}/api/v1/auth/logout-all`, {
      headers: { Cookie: `refresh_token=${refreshToken}; csrf_token=${csrfToken}` },
    });
    assertNo5xx(missingCsrf.status(), "logout-all missing csrf");
    expect(missingCsrf.status()).toBe(403);

    const refreshRes = await bare.post(`${API_BASE_URL}/api/v1/auth/refresh`, {
      data: { refresh_token: refreshToken },
    });
    assertNo5xx(refreshRes.status(), "refresh after missing csrf logout-all");
