...
test.describe("Atomic regenerate: apply-generation retries transient 429", () => {
  test.describe.configure({ mode: "serial" });

  test("retries apply-generation on 429 and closes modal after success", async ({ page }) => {
    const poseId = getCorePoseIdA();
    test.skip(!poseId, "Core seed pose not available");

    const taskId = `atomic_apply_retry_429_${Date.now()}`;
    ...
    await page.getByTestId("pose-regenerate-feedback").fill("atomic: apply retries 429");
