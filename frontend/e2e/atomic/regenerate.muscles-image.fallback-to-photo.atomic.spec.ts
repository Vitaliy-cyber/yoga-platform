...
test.describe("Atomic regenerate: muscles overlay broken should fall back to photo", () => {
  test.describe.configure({ mode: "serial" });

  test("when active overlay is Muscles and muscle image fetch fails, use photo as fallback", async ({
    page,
  }) => {
    ...
    let muscleFetches = 0;
    let photoFetches = 0;
    // Make fetchImageAsFile deterministic and simulate broken muscles overlay.
    await page.route("**/storage/**", async (route) => {
      ...
      if (url.toLowerCase().includes("schema")) {
        await route.fulfill({ status: 404, body: "Atomic: missing schema" });
        return;
      }

      if (url.toLowerCase().includes("muscle")) {
        muscleFetches += 1;
        await route.fulfill({ status: 404, body: "Atomic: broken muscles overlay" });
        return;
      }
    ...
    await page.route("**/api/v1/generate", async (route) => {
      ...
      await route.fulfill({
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          task_id: `atomic_muscles_fallback_${Date.now()}`,
          status: "pending",
          progress: 0,
    ...
    // Switch overlay to "Muscles" so RegenerateModal gets activeTab="muscles".
    ...
    await page.getByTestId("pose-regenerate-feedback").fill("atomic: muscles broken fallback to photo");
