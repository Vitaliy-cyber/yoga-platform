...
async function createPose(accessToken: string, code: string, categoryId: number): Promise<number> {
  const res = await authedFetch(accessToken, "/api/v1/poses", {
    ...
    body: JSON.stringify({
      code,
      name: `atomic json cross-user ${Date.now()}`,
      category_id: categoryId,
    }),
  });
  assertNo5xx(res.status, "create pose for cross-user export");
  expect(res.status).toBe(201);
  const json = (await res.json()) as { id?: number };
  expect(typeof json.id).toBe("number");
  return json.id as number;
...
test.describe("Atomic export poses JSON cross-user category (break-it; no 5xx)", () => {
  test.describe.configure({ mode: "serial" });

  test("export with чужий category_id returns 404", async () => {
    const userA = await loginWithToken(makeIsolatedToken("export-json-cross-a"));
    const userB = await loginWithToken(makeIsolatedToken("export-json-cross-b"));

    const suffix = Date.now().toString(36).slice(-8);
    const categoryName = `CrossCat-${suffix}`;
    const code = `JSON_X_${suffix}`.slice(0, 20);
    ...
    try {
      ...
      const res = await authedFetch(userB.accessToken, `/api/v1/export/poses/json?category_id=${categoryId}`);
      assertNo5xx(res.status, "export poses/json cross-user category");
