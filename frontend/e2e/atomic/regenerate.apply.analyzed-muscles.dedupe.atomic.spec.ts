...
test.describe("Atomic apply-generation: dedupe analyzed_muscles_json", () => {
  test.describe.configure({ mode: "serial" });

  test("apply-generation succeeds when analyzed_muscles_json has duplicate muscles (no 409 loop)", async () => {
    ...
    const code = `DEDUP_${Date.now().toString(36).slice(-10)}`.slice(0, 20);
    const createRes = await authedFetch(accessToken, "/api/v1/poses", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code, name: `Atomic Dedup ${code}` }),
    });
    ...
    try {
      ...
      const genRes = await authedFetch(accessToken, `/api/v1/generate/from-pose/${poseId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ additional_notes: "atomic: seed for dedupe test" }),
      });
      ...
      // Duplicate the same muscle multiple times; this can happen with noisy model output.
      // If not deduped, DB insert violates composite PK (pose_id, muscle_id) and apply loops on 409.
      const dupes = JSON.stringify([
        { name: "quadriceps", activation_level: 70 },
        { name: "quadriceps", activation_level: 80 },
        { name: "quadriceps", activation_level: 90 },
        { name: "hamstrings", activation_level: 40 },
        { name: "hamstrings", activation_level: 60 },
      ]);
      patchAnalyzedMusclesJson(getDbPath(), taskId, dupes);

      const applyRes = await authedFetch(accessToken, `/api/v1/poses/${poseId}/apply-generation/${taskId}`, {
        method: "POST",
      });
      assertNo5xx(applyRes.status, "apply-generation (dupes)");
      expect(applyRes.status).toBe(200);

      const pose = (await safeJson(applyRes)) as { muscles?: Array<{ muscle_name?: string; activation_level?: number }> } | undefined;
      const muscles = pose?.muscles || [];
      const names = muscles.map((m) => String(m.muscle_name || ""));
