...
test.describe("Atomic regenerate: from-pose 500 should NOT fall back to upload /generate", () => {
  test.describe.configure({ mode: "serial" });

  test("does not call /generate when /generate/from-pose returns 500", async ({ browser }) => {
    ...
    const token = makeIsolatedToken(`regen-500-no-fallback-${Date.now()}`);
    const { accessToken } = await loginWithToken(token);
    expect(accessToken).toBeTruthy();
    await uiLoginWithToken(page, token);

    const code = `R500_${Date.now().toString(36).slice(-10)}`.slice(0, 20);
    const createRes = await authedFetch(accessToken, "/api/v1/poses", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code, name: `Atomic Regen 500 NoFallback ${code}` }),
    });
    assertNo5xx(createRes.status, "create pose (regen 500 no-fallback suite)");
    ...
    await page.route(`**/api/v1/poses/${poseId as number}`, async (route) => {
      ...
      assertNo5xx(real.status, "get pose (regen 500 no-fallback suite)");
    ...
    await page.route(`**/api/v1/generate/from-pose/${poseId as number}`, async (route) => {
      ...
      await route.fulfill({
        status: 500,
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ detail: "Atomic: internal error" }),
      });
    });
    ...
    try {
      ...
      await page.getByTestId("pose-regenerate-feedback").fill("atomic: do not fallback on 500");
      await start.click();
