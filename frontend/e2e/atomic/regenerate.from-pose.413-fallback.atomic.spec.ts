...
test.describe("Atomic regenerate: from-pose 413 should fall back to upload /generate", () => {
  test.describe.configure({ mode: "serial" });

  test("falls back to /generate when /generate/from-pose returns 413", async ({ page }) => {
    ...
    await page.route(`**/api/v1/generate/from-pose/${poseId as number}`, async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({ detail: "Atomic: payload too large" }),
      });
    });
    ...
    await page.getByTestId("pose-regenerate-feedback").fill("atomic: 413 fallback");
