...
class TestCompleteWorkflows:
    ...
    @pytest.mark.asyncio
    async def test_complete_pose_management_workflow(self, client: AsyncClient):
        ...
        category_poses = await client.get(f"/api/poses/category/{category_id}")
        ...
        # 5. Search for pose
        search_response = await client.get("/api/poses/search?q=warrior")
        assert search_response.status_code == 200
        assert len(search_response.json()) == 1

        # 6. Get pose by code
        code_response = await client.get("/api/poses/code/WAR01")
    ...
    @pytest.mark.asyncio
    async def test_pose_with_muscles_workflow(self, client: AsyncClient):
        ...
        legs_response = await client.get("/api/muscles?body_part=legs")
        assert legs_response.status_code == 200
        leg_muscles = legs_response.json()

        # Get core muscles
        core_response = await client.get("/api/muscles?body_part=core")
    ...
    @pytest.mark.asyncio
    async def test_multiple_categories_and_poses_workflow(self, client: AsyncClient):
        ...
        for i, cat in enumerate(categories):
            response = await client.get(f"/api/poses/category/{cat['id']}")
            assert response.status_code == 200
            expected_count = sum(1 for p in pose_data if p[2] == i)
            assert len(response.json()) == expected_count

        # Verify total count
        all_poses = await client.get("/api/poses")
        assert len(all_poses.json()) == 6

        # Search across categories
        search_response = await client.get("/api/poses/search?q=a")
        assert search_response.status_code == 200
        # Should find: Mountain, Warrior II, Lotus, Eagle, Headstand (all have 'a')

    @pytest.mark.asyncio
    async def test_category_deletion_cascade(self, client: AsyncClient):
        """Test that deleting category properly handles associated poses."""
        # Create category
        cat_response = await client.post("/api/categories", json={"name": "To Delete"})
...
class TestImageUploadWorkflow:
    ...
    async def test_upload_and_retrieve_schema(
        ...
        # 1. Create pose
        pose_response = await client_with_mocked_storage.post(
            "/api/poses",
        ...
        upload_response = await client_with_mocked_storage.post(
        ...
        get_response = await client_with_mocked_storage.get(f"/api/poses/{pose_id}")
...
class TestGenerationWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_generation_full_flow(self, client_with_mocked_storage: AsyncClient):
        ...
        # 2. Check status
        status_response = await client_with_mocked_storage.get(
...
class TestDataIntegrity:
    ...
    @pytest.mark.asyncio
    async def test_pose_code_uniqueness(self, client: AsyncClient):
        """Test that pose codes are unique across all poses."""
        # Create first pose
        await client.post("/api/poses", json={"code": "UNIQUE01", "name": "Pose 1"})
    ...
    @pytest.mark.asyncio
    async def test_category_name_uniqueness(self, client: AsyncClient):
        """Test that category names are unique."""
        await client.post("/api/categories", json={"name": "Unique Category"})
    ...
    @pytest.mark.asyncio
    async def test_muscle_id_validation(self, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_activation_level_boundaries(self, client: AsyncClient):
        """Test that activation levels are validated."""
        await client.post("/api/muscles/seed")
        muscles = (await client.get("/api/muscles")).json()
    ...
    @pytest.mark.asyncio
    async def test_pose_updated_at_changes(self, client: AsyncClient):
...
class TestPaginationAndFiltering:
    ...
    @pytest.mark.asyncio
    async def test_poses_pagination(self, client: AsyncClient):
        ...
        # Get first page
        page1 = await client.get("/api/poses?skip=0&limit=10")
        assert len(page1.json()) == 10

        # Get second page
        page2 = await client.get("/api/poses?skip=10&limit=10")
    ...
    @pytest.mark.asyncio
    async def test_muscles_body_part_filter(self, client: AsyncClient):
        """Test filtering muscles by body part."""
        await client.post("/api/muscles/seed")

        # Test each body part
        for body_part in ["legs", "core", "back", "arms", "chest", "shoulders"]:
            response = await client.get(f"/api/muscles?body_part={body_part}")
            assert response.status_code == 200
...
class TestErrorRecovery:
    ...
    @pytest.mark.asyncio
    async def test_transaction_rollback_on_error(self, client: AsyncClient):
        ...
        cat_check = await client.get(f"/api/categories/{category_id}")
