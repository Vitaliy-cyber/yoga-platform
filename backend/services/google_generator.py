...
class GoogleGeminiGenerator:
    ...
    STUDIO_PHOTO_PROMPT = (
        ...
        "UNIVERSAL GEOMETRY LOGIC (PRECISION MODE):"
        "1. **THE \"AIR GAP\" RULE (Elevation Check):** Look at the space between the subject's pelvis and the floor/heels. Replicate this vertical distance exactly (grounded vs lifted)."
        "2. **NEGATIVE SPACE CALIBRATION (Crucial):** Look at the empty shapes/gaps *between* the limbs and the body (e.g., the triangle of air under a bent arm, the space between knees). You must replicate the exact shape and size of these empty \"holes\". If you get the empty space right, the body geometry will be correct."
        "3. **EXACT JOINT ANGLES:** Do not generalize. Trace the precise angle of every visible joint (elbows, knees, spine curvature). If an elbow is bent at an acute angle in the reference, it must be the exact same acute angle in the output."
